...
- **Daily Mission status (VocabaReminder)** (sam 21.09, обновлено 21.09): `missionStatus(today)` → {learned, goal, debt, completedToday, reviewDone, learnDone}; лёгкий пересчёт, кормит in-app баннер. **ОС-нотификации (notify/maybeRemind/requestReminderPermission/startReminders + таймеры) УБРАНЫ 21.09** (запрос разрешения при каждом запуске раздражал). `window.VocabaReminder = { missionStatus }`. `window.VocabaMission = {render: renderDailyMission}` — app.js зовёт после loadData (фикс F1 гонки: datacare.init бежит до резолва loadData → Mission стейл).
- **In-app reminder banner** (21.09): видимая полоса `#dm-reminder` на дашборде (datacare renderReminderBanner) — «⏰ Daily mission not done — N reviews · M/K new words left» + кнопка Practice + ✖ (скрыть до завтра, localStorage `vocaba_reminder_dismissed`=today). Появляется, когда Mission не закрыта; ре-рендер из renderDailyMission. **Заменила ОС-нотификации** (не требует прав). Тест check-datacare §17.
- **Trash (мусорка)** (21.09, datacare `VocabaTrash`): перенос слова из `appState.cards` в localStorage `vocaba_trash` (полный снимок) — карточка автоматически выпадает из всех SRS-очередей (build*Queue итерируют appState.cards) и прячется из словаря. `validateState` пропускает shrink через `removedIds`. Словарь рисует псевдо-карточку 🗑️ Trash в конце сетки (`trashCardEl`): видна, если в мусоре есть слова И (нет поиска/фильтра ИЛИ запрос содержит 'trash'); клик → модалка `#modal-trash` со списком + ↩ Restore. Перенос инициируется кнопкой «🗑️ Move to Trash» в Card Stats (📊). Тест check-datacare §18.
