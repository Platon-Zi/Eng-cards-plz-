 ...
 * гарантируются ядром, а не здешним кодом. Исключение — learn (20.09): новое
 * слово намеренно идёт ПАРОЙ EN→RU + RU→EN подряд, чтобы заучивание сразу
 * закладывало обе стороны; в повторениях стороны по-прежнему разнесены.
 */
...
// (22.09, фидбек «проглатывает первые буквы слова») — очередь запросов речи:
// номер последнего вызова speakEnglish + settle-пауза после cancel().
var speakSeq = 0;
var SPEAK_SETTLE_MS = 90;

function speakEnglish(text) {
  if (!text) return;
  // cancel() рвёт текущее воспроизведение АСИНХРОННО: speak() в следующем тике
  // (прошлый фикс 21.09, setTimeout 0) всё равно иногда стартует «на лету» —
  // движок ещё разбирает отменённое слово, и первые буквы нового съедаются.
  // Лечение: 1) говорим только ПОСЛЕДНИЙ запрос (быстрые клики по разным
  // словам/кнопкам не накладываются друг на друга); 2) settle-пауза 90мс
  // после cancel — конвейер успевает закрыться до нового слова, для уха
  // пауза незаметна. Карточки/словарь/примеры звучат с ОБЫЧНОЙ скоростью
  // (1.0) — по запросу пользователя (19.09); spellingState.settings.speechRate
  // намеренно НЕ читаем: тот слайдер — учебный темп диктанта Spelling Studio.
  var my = ++speakSeq;
  try { window.speechSynthesis.cancel(); } catch (e) {}
  setTimeout(function () {
    if (my !== speakSeq) return;   // пришёл более новый запрос — этот молчит
