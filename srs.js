...
(function (globalScope) {
  ...
  function buildLearnQueue(cards, today, opts) {
    ...
    words.forEach(function (card) {
      items.push(makeItem(card, 'en_ru', today, salt, 'learn'));
    });
    // (21.09, фидбек пользователя) Заучивание = ОДНА карточка на слово, EN→RU.
    // Обратная сторона (RU→EN) НЕ показывается в learn — слово не дублируется.
    // Отвечая EN→RU, банк-слово переходит в ACTIVE: activateCard ставит ОБА
    // вектора на level 0, next_review = сегодня — слово «разделяется на 2 стороны»,
    // и обе попадают в ПОВТОРЕНИЕ (review-очередь, с разнесением spreadSameCard).
    // Так learn остаётся чистым вводным показом (одно слово один раз), а обе
    // стороны закрепляются повторением. (Раньше учили парами — пользователь
    // счёл дубль одного слова лишним и указал, что видел только EN→RU.)
    return items;
  }
